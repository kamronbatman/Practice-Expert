...
module.exports = function (patId, fromDate, toDate, dctId, fclId, callback) {
    ...
    var sql1 = "declare  @tempTableA table \
                ...
                where \
                ( \
                	(tmpa.PatId = tmpf.PatId) \
                	and \
                	( \
                		(isNull (tmpf.StartDate, tmpf.DateofVisit) >= isNull (tmpa.StartDate, tmpa.DateofVisit)) and (isNull (tmpf.StartDate, tmpf.DateofVisit)<=isNull (tmpa.StopDate, tmpa.DateofVisit)) \
                		or \
                		( \
                			DATEDIFF(day,isNull (tmpf.StartDate, tmpf.DateofVisit), isNull (tmpa.StartDate, tmpa.DateofVisit)) = 0 \
                		) \
                	) \
                )";
            ...
            cqmGuideController.cqmCheck(sql1, function(err, record){
              ...
              {
                callback({criteriaName: "Denominator Exceptions", isCriteriaQualify : false,  message : "Macular Exam Not Performed (Medical/Patient Reason)"});
              }
            });
