...
module.exports = function (patId, fromDate, toDate, dctId, fclId, callback) {
  ...
  var sql1 = "declare  @tempTableA table \
              ...
              select tmpa.PatId from @tempTableC tmpc, @tempTableD tmpd, @tempTableA tmpa \
              where \
              (tmpa.PatId = '"+patId+"') \
              and \
              ( \
              ...
              and \
              ( \
                DATEDIFF(day,isNull (tmpd.StartDate, tmpd.DateofVisit), isNull (tmpa.StartDate, tmpa.DateofVisit)) = 0 \
              )";
        ...
        cqmGuideController.cqmCheck(sql1, function(err, record){
            if(record && record.length > 0)
            {
              ...
              callback({criteriaName: "Numerator", isCriteriaQualify : false,  message : "Macular Exam, Result: Macular Edema Findings Absent/Present during visit"});
            }
        });
