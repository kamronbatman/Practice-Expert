...
module.exports = function (patId, fromDate, toDate, dctId, fclId, callback) {
  ...
  var sql1 = "declare  @tempTableA table \
              ...
              ( \
                ...
                and	((select dbo.xrxGetPatAge(pat.Birthdate,null, cqm.DateOfVisit)) <= 64) \
              ) \
              ...
              ( \
                ...
                and \
              	( \
              		(cqm.patId in (select PatId from @tempTableD where ((BMI >= 18.5) and (BMI < 25)) )) \
              		or \
              		( \
              			(cqm.patId in (select PatId from @tempTableD where ((BMI >= 25)) )) \
              			and \
              			(cqm.patId in (select PatId from @tempTableDE )) 	\
              		) \
              		or \
              		( \
              			(cqm.patId in (select PatId from @tempTableD where ((BMI < 18.5)) )) \
              			and \
              ...
              cqmGuideController.cqmCheck(sql1, function(err, record){
                  if(record && record.length > 0)
                  {
                    ...
                    callback({criteriaName: "Numerator", isCriteriaQualify : false,  message : "during encounter patien age is between 18 to 64 and, \
                                                                                                <p align='justify' class='text-danger'>BMI is in between 18.5 and 25</p> \
                                                                                                <p align='justify' class='text-danger'>Or BMI >= 25 and  a follow-up plan is documented within 6 moths of current encounter.  </p> \
                                                                                                <p align='justify' class='text-danger'>Or BMI < 18.5 and  a follow-up plan is documented within 6 moths of current encounter.  </p>"});
                  }
              });
