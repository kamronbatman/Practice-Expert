...
module.exports = function (patId, fromDate, toDate, dctId, fclId, callback) {
  ...
  var sql1 = "  select distinct med.PatId \
                ...
                group by med.PatId \
                having (COUNT(med.PatId) >= 1)  \
              ";
              console.log(sql1);
        cqmGuideController.cqmCheck(sql1, function(err, record){
            if(record && record.length > 0)
            {
              callback({criteriaName: "Numerator1", isCriteriaQualify : true,  message : null});
            }
            else if(err) {
              callback({criteriaName: "Numerator1", isCriteriaQualify : false,  message : err});
            }
            else {
              callback({criteriaName: "Numerator1", isCriteriaQualify : false,  message : "High Risk Medications for the Elderly (Count >= 1)."});
            }
        });
}
