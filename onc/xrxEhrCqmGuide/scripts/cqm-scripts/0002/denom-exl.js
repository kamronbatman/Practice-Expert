...
module.exports = function (patId, fromDate, toDate, dctId, fclId, callback) {
  ...
  var sql1 = "declare  @tempTableC table \
              ...
              select tmpe.PatId \
              from @tempTableE tmpe, @tempTableC tmpc \
              where \
              (tmpc.PatId = '"+patId+"') \
              and \
              (tmpc.PatId = tmpe.PatId) \
              and \
              ( \
              	isNull (tmpe.StartDate, tmpe.DateofVisit) < isNull (tmpc.StartDate, tmpc.DateofVisit) \
              	and \
              	(DATEDIFF(day, isNull (tmpe.StartDate, tmpe.DateofVisit), isNull (tmpc.StartDate, tmpc.DateofVisit))) <= 30 \
              )";
        ...
        cqmGuideController.cqmCheck(sql1, function(err, record){
              ...
              {
                  callback({criteriaName: "Denominator Exclusions", isCriteriaQualify : false,  message : "Medication, Active: Antibiotic Medications  within 30 day(s) starts before start of Active: Acute Pharyngitis OR Tonsillitis"});
              }
        });
