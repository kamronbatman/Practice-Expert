...
module.exports = function (patId, fromDate, toDate, dctId, fclId, callback) {
    ...
    var sql1 = "declare @InitialPopOrDenom table \
                ...
                insert into @Nume \
                select tc.PatId from @InitialPopOrDenom pop,  @TableC tc \
                where \
                (pop.PatId = tc.PatId ) \
                and  not \
                ( \
                	((isNull (tc.StartDate, tc.DateofVisit)) >= (isNull (pop.StartDate, pop.DateofVisit))) \
                	and \
                	isNull (tc.StartDate, tc.DateofVisit) <= dateadd(day, 3 ,  isNull (pop.StartDate, pop.DateofVisit)) \
                ) \
                insert into @Nume \
                select distinct pop.PatId from  @InitialPopOrDenom pop \
                except \
                select PatId from @TableC \
        ...
        cqmGuideController.cqmCheck(sql1, function(err, record){
            if(record && record.length > 0)
            {
              ...
              callback({criteriaName: "Numerator", isCriteriaQualify : false,  message : "Medication, Order: Antibiotic Medications should not start within 3 days after Diagnosis, Active: Upper Respiratory Infection"});
            }
        });
