...
exports.getOptions = function(userId, callback) {
    ...
    parameters = [
        ...
        {parameter: 'OptName', type: 'VarChar', value: 'Clinical Summaries'},
...
exports.saveOptions = function(userId, data, callback) {
  ...
  var sql = "UPDATE xrxEhr_options2 SET  "+ colStr +"  WHERE (UserId = '"+ userId +"') AND (OptName = 'Clinical Summaries') AND (isNull(VisitType, '') = '') " ;
...
exports.getClinicalSummariesTemplate = function (callback) {

  var sql = "select RecNo, FormHtml from xrxForms_List where FormName = 'Clinical Summaries'";
...
exports.getClinicalSummariesData = function (patId, dateOfVisit, license, userId, callback) {

  var sql = "select RecNo, FormSql from xrxForms_List where FormName = 'Clinical Summaries'";
  ...
  dbhelper.query(sql, function (err, record) {
      ...
      else
      {
          if(record && record.length > 0)
          {
              ...
              {
                  var sqlClinicalSummaries = record[0].FORMSQL.value;
                  ...
                  parameters = [
                    ...
                    {parameter: 'UserId', type: 'VarChar', value: userId},
                    {parameter: 'DctId', type: 'VarChar', value: cmGlb.dctId},
                    {parameter: 'License', type: 'VarChar', value: license}
                  ];

                  dbhelper.queryWithParams(sqlClinicalSummaries, parameters, function (err1, record1) {
              ...
              {
                  callback("Function : getClinicalSummariesData -> No SQL Value Found for Clinical Summaries in Record", null);
              }

          }
          else
          {
            callback("Function : getClinicalSummariesData -> No Record Found in xrxForms_List", null);
          }
...
exports.sendSummariesToPortal = function(patId, userId, dateOfVisit, dctId, img, callback){


  var noteType = 'ToPat';
  var notes = 'Please click on the button (Open Attached Pdf)  to view your Summaries document.';
  var subject = 'Clinical Summaries';
    ...
    //Send email
     ...
     var mailOptions = {
       ...
       html: "<p>Dear "+ firstName +",</p> \
              <p>Clinical Summary is now available for "+ patientName +". <br /> \
