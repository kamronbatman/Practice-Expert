...
var sqlTrx = require(appPath + '/dc/xrxTransaction/controllers/transactionController.js');

var DictionaryTable = React.createClass({
getDefaultProps: function() {
    return {
        sortColumnName: "TRANSACTIONDATE",
...
_getRowCount: function() {
    var self = this;
    sqlTrx.xrxTransactionGetCount(self._getWhereClause() , function(err, count){
      if(err){
        console.log(err);
        self._setRowCount(0);
      }
...
_setRowCount: function(count) {
    ...
    this.setState({	size: count, dataList: new DataListStore(count, this._getWhereClause(), this.state.sortBy, this.state.sortType,  10, sqlTrx.xrxTransactionGetList)});
    return;
},
...
_onSortChange: function(columnKey, sortDir) {
  ...
  this.setState({	sortBy: this.state.sortBy, sortType: this.state.sortType,
                 dataList: new DataListStore(this.state.size, this._getWhereClause(), this.state.sortBy, this.state.sortType,  10, sqlTrx.xrxTransactionGetList)});
},
...
_openEditWindow: function(recNo){
    ...
    let win = new BrowserWindow({ width: 700, height: 650, center: true, parent : BrowserWindow.getFocusedWindow(), modal : true, show: false });
...
render: function(){
          ...
          <div>
              ...
              <div className="row">
                  ...
                  <Column columnKey="TRANSACTIONDATE"
                   header={<SortHeaderCell onSortChange={this._onSortChange} sortDir={(this.state.sortBy === 'TRANSACTIONDATE' ? this.state.sortType : '')}>Trans. Date</SortHeaderCell>}
                  ...
                  <Column columnKey="DCTID"
                   ...
                   <Column columnKey="PATID"
                    header={<SortHeaderCell onSortChange={this._onSortChange} sortDir={(this.state.sortBy === 'PATID' ? this.state.sortType : '')}>Pat Id</SortHeaderCell>}
                    cell={this._dataCell} width={50} flexGrow={1}/>

                  <Column columnKey="FROMDATE"
                   header={<SortHeaderCell onSortChange={this._onSortChange} sortDir={(this.state.sortBy === 'FROMDATE' ? this.state.sortType : '')}>From Date</SortHeaderCell>}
                   cell={this._dateCell} width={50} flexGrow={1}/>

                 <Column columnKey="TODATE"
                    header={<SortHeaderCell onSortChange={this._onSortChange} sortDir={(this.state.sortBy === 'TODATE' ? this.state.sortType : '')}>To Date</SortHeaderCell>}
                    cell={this._dateCell} width={50} flexGrow={1}/>

                  <Column columnKey="PICKUPDATE"
                   header={<SortHeaderCell onSortChange={this._onSortChange} sortDir={(this.state.sortBy === 'PICKUPDATE' ? this.state.sortType : '')}>Pickup Date</SortHeaderCell>}
                   cell={this._dateCell} width={50} flexGrow={1}/>

                 <Column columnKey="CREATEDDATE"
                    header={<SortHeaderCell onSortChange={this._onSortChange} sortDir={(this.state.sortBy === 'CREATEDDATE' ? this.state.sortType : '')}>Created Date</SortHeaderCell>}
                    cell={this._dateCell} width={50} flexGrow={1}/>

                  <Column columnKey="DELETEDDATE"
                     header={<SortHeaderCell onSortChange={this._onSortChange} sortDir={(this.state.sortBy === 'DELETEDDATE' ? this.state.sortType : '')}>Deleted Date</SortHeaderCell>}
                     cell={this._dateCell} width={50} flexGrow={1}/>
